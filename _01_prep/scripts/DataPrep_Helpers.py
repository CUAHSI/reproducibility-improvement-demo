...
def load_csv(file_path: str,
             ...
             use_cols: list = None) -> pd.DataFrame:
    """
    ...
    Args:
        ...
        date_column (str, optional): The name of the column to convert to datetime.
...
def resample_and_interpolate(df: pd.DataFrame,
                             resample_freq: str,
                             on_column: str = None,
                             interpolation_method: str = 'linear',
                             interpolation_limit: int = None) -> pd.DataFrame:
    """
...
def remove_outliers_by_quantile(df: pd.DataFrame,
                                columns: list,
                                lower_quantile: float = 0.005,
                                upper_quantile: float = 0.995) -> pd.DataFrame:
    """
...
def apply_range_filter(df: pd.DataFrame,
                       column: str,
                       lower_bound: float = None,
                       upper_bound: float = None,
                       replace_value=np.nan) -> pd.DataFrame:
    """
...
def filter_by_datetime_range(df: pd.DataFrame,
                             ...
                             start_hour: int = None,
                             end_hour: int = None,
                             exclude_year: int = None) -> pd.DataFrame:
    """
...
def calculate_rolling_sums(df: pd.DataFrame, 
                           columns: list, 
                           window_days: list) -> pd.DataFrame:
    """
...
def calculate_cumulative_sum_by_year(df: pd.DataFrame, 
                                     column: str, 
...
def calculate_doy(df: pd.DataFrame, 
...
def clean_data_by_masking_values(df: pd.DataFrame,
                                 columns: list,
