...
def export_river_monticello_results(df_filtered, df_cluster_means, df_flow_prob,
    ...
    df_filtered.to_csv(os.path.join(output_folder, 'river_monticello_filtered_data.csv'), index=True)

    # Save cluster means
    df_cluster_means.to_csv(os.path.join(output_folder, 'river_monticello_cluster_means.csv'), index=True)

    # Save flow probabilities
    df_flow_prob.to_csv(os.path.join(output_folder, 'river_monticello_flow_prob.csv'), index=False)

    # Save column names
    with open(os.path.join(output_folder, 'river_monticello_colnames.txt'), 'w') as f:
...
def run_river_monticello_analysis(nc=6, data_folder='DATA/Processed/',
    ...
    export_river_monticello_results(df_filtered, df_cluster_means, df_flow_prob,
                                    colnames_responses, colnames_drivers)


if __name__ == '__main__':
    nc_value = 6 # Example number of clusters
    run_river_monticello_analysis(nc=nc_value)
